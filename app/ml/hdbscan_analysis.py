...
def run_hdbscan_and_plot(df: pd.DataFrame, target_variance: float = 0.90, min_cluster_size: int = None, min_samples: int = None):
    ...
    # --- CORRECCIÓN 1: Límite de filas reducido para pruebas ---
    # (En producción, deberías usar 20 o más y tener más datos)
    if len(X_num) < 10:
        raise ValueError(f"No hay suficientes filas ({len(X_num)}) para clusterizar. Se requieren al menos 10.")
    ...
    # Asegura que n_components no sea mayor que el número de features o muestras
    n_components_pca = min(pca_limit, len(X_num))
    
    pca = PCA(n_components=n_components_pca)
    X_pca_full = pca.fit_transform(X_scaled)
    
    # Intenta alcanzar la varianza objetivo, pero sin exceder los límites
    explained_variance_ratio_cumulative = np.cumsum(pca.explained_variance_ratio_)
    try:
        n_pcs_target = np.where(explained_variance_ratio_cumulative >= target_variance)[0][0] + 1
    except IndexError:
        # Si no se alcanza la varianza, usa todos los componentes disponibles
        n_pcs_target = n_components_pca
        
    n_pcs = min(n_pcs_target, pca_limit)
    ...
    # --- CORRECCIÓN 2: Asegurar que los parámetros no sean 0 ---
    # Asegura que los valores estén en un rango razonable y sean al menos 1 o 2.
    min_cluster_size = max(2, min(min_cluster_size, n_samples_total // 10))
    min_samples = max(1, min(min_samples, n_samples_total // 20))

    # Asegura que min_samples no sea mayor que min_cluster_size
    ...
    loadings = pca2.components_ # Matriz de rotación
    ...
    # Asegura que el colormap tenga suficientes colores
    num_colors = max(2, n_clusters)
    ...
    if np.any(mask_cluster): # Solo dibuja clusters si existen
